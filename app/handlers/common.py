...
async def cmd_start(message: Message, state: FSMContext, is_admin: bool, **kwargs) -> None:
    await state.clear()
    name = message.from_user.full_name if message.from_user else "کاربر"
    await message.answer(
        f"👋 سلام <b>{name}</b>!\n\n"
        "به ربات میم و تبلیغات خوش آمدید.\n"
...
async def back_to_main(message: Message, state: FSMContext, is_admin: bool, **kwargs) -> None:
    await state.clear()
    await message.answer("منوی اصلی:", reply_markup=main_menu_kb(is_admin=is_admin))


@router.message(F.text == "❌ انصراف")
async def cancel_action(message: Message, state: FSMContext, is_admin: bool, **kwargs) -> None:
    await state.clear()
